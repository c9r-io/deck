...
// Link releases verify the public buffer against the press snapshot, surviving
// unchanged row repaints. Link diagnostics contain IDs, enums and counts only.
...
function wireTerminalLinks(pane, card) {
  ...
  const log = (detail, a, b, trace = context()) => uev('terminal-link', detail, a, b, trace);
  const scan = lineNo => {
  ...
  const up = e => {
    ...
    term.clearSelection();
    const c = card();
    showLinkCtx(e, current.kind, current.text, c ? c.dir : ctx.HOME, c ? c.id : null,
      current.lookback, ended.trace);
