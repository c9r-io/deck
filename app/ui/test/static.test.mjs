...
test('the canonical dictionary has no unused keys outside documented dynamic families', () => {
  ...
  const dynamic = /^(?:attention\.column|attention\.filter|automation\.run|automation\.wd|board\.default|buffer\.state|mcp\.tunnelState|session\.status|settings\.shortcut|notice|tmux\.notice|voice\.phase|voice\.error|voice\.notice)\./;
