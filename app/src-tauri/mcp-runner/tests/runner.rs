...
fn large_scripts_and_full_reads_cross_the_accepted_socket_intact() {
    ...
    for round in 0..20 {
        ...
        let started = call(
            &socket,
            json!({"kind":"exec","job_id":job,"request_hash":format!("hash_{round}"),"script":script,"cwd":root,"wait_ms":3000,"context":context}),
        );
...
fn stop_escalates_and_reaps_the_job_group() {
    ...
    call(
        &socket,
        json!({"kind":"exec","job_id":"job_stop","request_hash":"hash_stop","script":script,"cwd":root,"wait_ms":0,"context":context('a')}),
    );
...
fn runner_termination_kills_the_live_job_group() {
    ...
    call(
        &socket,
        json!({"kind":"exec","job_id":"job_term","request_hash":"hash_term","script":script,"cwd":root,"wait_ms":0,"context":context('b')}),
    );
    let pid = job_pid(&root);
...
fn human_interrupt_key_reaches_the_job_group_not_the_runner() {
    ...
    call(
        &socket,
        json!({"kind":"exec","job_id":"job_human","request_hash":"hash_human","script":script,"cwd":root,"wait_ms":0,"context":context('c')}),
    );
    let pid = job_pid(&root);
...
fn a_job_stopped_by_job_control_is_reported_and_still_stoppable() {
    ...
    call(
        &socket,
        json!({"kind":"exec","job_id":"job_tstp","request_hash":"hash_tstp","script":"kill -STOP $$; sleep 600","cwd":root,"wait_ms":0,"context":context('d')}),
    );
    let limit = Instant::now() + Duration::from_secs(3);
    loop {
