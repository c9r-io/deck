...
The separate `testPairedScratchpadNoteCRUD` test is an explicitly destructive sandbox-card check. In the copied UI-test plan, also set `DECK_UI_ALLOW_NOTE_MUTATION=1` and a fresh `DECK_UI_NOTE_MARKER=DECK-UI-NOTE-<unique-run>`, then select only `DeckConnectorUITests/ConnectorUITests/testPairedScratchpadNoteCRUD`. It adds that marker once, proves the edit caret is at the end with an unsaved probe, clears the bounded printable-ASCII marker, accepts either an absent or empty accessibility value for the cleared SwiftUI editor, enters the marker plus `-edited`, and guards that exact value before saving. It then deletes only the matching row. A pending add or delete uses Check original operation once; a pending edit acknowledges the sheet and performs one background/activate cycle so the original operation can resolve, without saving again. On failure, do not rerun with the same marker or delete it blindly; leave it for host-side investigation and use a fresh marker only after explicit authorization. Require 1 passed, 0 skipped, and 0 failed.
