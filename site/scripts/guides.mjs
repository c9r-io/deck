...
<p class="guide-source">${zh ? '内容基于 deck 0.6.5 的功能，适用于 0.6.6。技术细节：' : 'Based on deck 0.6.5 functionality; also applies to 0.6.6. Technical reference: '}<a href="${config.githubUrl}/blob/${config.guideRef}/README.md">README ↗</a> · <a href="${config.feedbackUrl}">${zh ? '反馈问题' : 'Report an issue'}</a></p></main>
<aside class="guide-toc"><nav aria-label="${zh ? '本页目录' : 'On this page'}"><p>${zh ? '本页内容' : 'On this page'}</p>${toc}</nav></aside>
</div><footer class="site-footer"><div class="shell footer"><a class="brand" href="${home}">▦ deck</a><span>${zh ? '少惦记任务，知道何时该你接手。' : 'Less checking. Know when to step in.'}</span><div class="footer-links"><a href="${home}privacy/">${zh ? '隐私' : 'Privacy'}</a><a href="${config.githubUrl}">GitHub</a></div></div></footer>
</body></html>`;
